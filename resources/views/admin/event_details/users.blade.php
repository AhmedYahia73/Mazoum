...
@section('content')
    ...
    <div class="row justify-content-center" style="margin-top: 20px">
        ...
        <div class="col-12">
          ...
          @if(!isset($is_qr_page) && isset($type) && $type != 'confirmed_event_details')
          <b>
                                        ...
                                        @foreach ($data as $user)
                                            ...
                                            <tr  class="{{ $row_class }}">
                                                ...
                                                <td style="text-align: center;">
                                              	...
                                              	<td style="text-align: center;">
                                                    {{ $user->qr_sent == 'yes' ? 'نعم': 'لا' }}
                                                </td>

                                              	<td style="text-align: center;">
                                                    {{ $user->scan == 'yes' ? 'نعم': 'لا' }}
                                                </td>

                                                <td>
												   {{ $user->confirmed_at != null ? Carbon\Carbon::parse($user->confirmed_at)->format('Y-m-d h:i A') : null }}
                                                </td>
	...
	<script>
        ...
        function sendMessageToSelected2() {
            swal({
              ...
              function (isConfirm) {
                if (isConfirm) {
                  $('#all_users').attr('action', '{{ url("admin/send-custom-message") }}').submit();
                }
              });
        }
        ...
        function sendCongratulationMessageToSelected2() {
            swal({
              ...
              function (isConfirm) {
                if (isConfirm) {
                  $('#all_users').attr('action', '{{ url("admin/send-congratulation-messages") }}').submit();
                }
              });
        }
